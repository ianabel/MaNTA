...
struct type_caster<GlobalState> {
	...
	static handle cast(const GlobalState& src,
...
PYBIND11_MODULE(MaNTA, m, py::mod_gil_not_used()) {
	...
	m.def("getNodes",
	      py::overload_cast<const std::vector<double>&, unsigned int>(
		  &getNodes),
	      py::return_value_policy::reference, "Get the points of a grid");
	m.def("getNodes",
	      py::overload_cast<Position, Position, Index, unsigned int>(
		  &getNodes),
	      py::return_value_policy::reference, "Get the points of a grid");
	...
	py::class_<TransportSystem, PyTransportSystem, py::smart_holder>(
	    ...
	    .def("SigmaFn",
		 py::overload_cast<Index, const State&, Position, Time>(
	    ...
	    .def_readwrite("isUpperDirichlet",
			   &PyTransportSystem::isUpperDirichlet)
	    .def_readwrite("isLowerDirichlet",
			   &PyTransportSystem::isLowerDirichlet)
	...
	py::class_<AdjointProblem, PyAdjointProblem, py::smart_holder>(
	    ...
	    .def_readwrite("spatialParameters",
			   &PyAdjointProblem::spatialParameters);

	py::class_<Grid>(m, "Grid")
	    .def(py::init<>(), py::return_value_policy::reference)
	    .def(py::init<Grid::Position, Grid::Position, Grid::Index, bool,
			  double, double>(),
	...
	py::class_<toml::value>(m, "TomlValue")
	    .def(py::init<>())
	    .def("__getitem__", [](const toml::value& v,
		    ...
		    if (result.is_none()) {
				...
				"Key " + key + " not found in TOML value.");
		    } else {
			    return result;
		    }
	    });
	...
	m.def("runner_ffi_ops", []() {
		py::dict ffi_ops;
		ffi_ops["get_solution_ffi"] =
		    EncapsulateFfiCall(get_solution_ffi_ops);
