...
PYBIND11_MODULE(MaNTA, m, py::mod_gil_not_used())
{
    ...
    .def("ComputePhysicsDerivatives", &TransportSystem::ComputePhysicsDerivatives)
		...
		.def("AuxG", py::overload_cast<Index, const State &, Position, Time>(&TransportSystem::AuxG))
    .def("AuxG_v", py::overload_cast<Index, const State &, Position, Time>(&TransportSystem::AuxG))
		.def("AuxGPrime", py::overload_cast<Index, State&, const State &, Position, Time>(&TransportSystem::AuxGPrime))
    .def("AuxGPrime_v", py::overload_cast<Index, GlobalState &, GlobalState const &, std::vector<Position> const &, Time>(&TransportSystem::AuxGPrime))
