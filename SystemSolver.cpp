...
SystemSolver::SystemSolver(Grid const& Grid, unsigned int polyNum, unsigned int N_cells, double Dt, Fn const& rhs, Fn const& Tau, Fn const& c, Fn const& kappa )
	: grid(Grid), k(polyNum), nCells(N_cells), dt(Dt), RHS(rhs), tau(Tau), c_fn(c), kappa_fn(kappa), u(grid,k), q(grid,k), dudt(grid,k), BCs (nullptr)
{
}
...
void SystemSolver::mapDGtoSundials(DGApprox& q, DGApprox& u, realtype* Y)
{
	...
	{
		q.coeffs.emplace_back( grid.gridCells[ i ], VectorWrapper( Y+i*(2*k+2), k+1 ));
		u.coeffs.emplace_back( grid.gridCells[ i ], VectorWrapper( Y+(i)*(2*k+2)+k+1, k+1 ));
	}
}
...
void SystemSolver::mapDGtoSundials(DGApprox& u, realtype* Y)
{
	...
	for(int i=0; i<nCells; i++)
	{
		VectorWrapper emptyVec( Y+i*(2*k+2), k+1 ); 
		u.coeffs.emplace_back( grid.gridCells[ i ], VectorWrapper ( Y+i*(2*k+2)+k+1, k+1 ));
		emptyVec.setZero();
	}
}

void SystemSolver::mapDGtoSundials(std::vector< Eigen::Map<Eigen::VectorXd> >& QU_cell, realtype* const& Y)
{
	for(int i=0; i<nCells; i++)
	{
		QU_cell.emplace_back( VectorWrapper( Y+i*(2*k+2), 2*k+2 ) );
	}
}
...
void SystemSolver::DGtoSundialsVecConversion(DGApprox delQ, DGApprox delU, N_Vector& delY)
{
	...
	{
		Interval const& I = grid.gridCells[ i ];
		qCellCoeffs = delQ.coeffs[ i ].second;
		uCellCoeffs = delU.coeffs[ i ].second;
		for (int j=0; j<k+1; j++)
		{
...
int residual(realtype tres, N_Vector Y, N_Vector dydt, N_Vector resval, void *user_data)
{
	...
	for ( unsigned int i=0; i < nCells; i++ )
	{
		...
		Eigen::Vector2d CqGuL;
		CqGuL = Lcell - system->C_cellwise[i]*tempQ.coeffs[ i ].second - system->G_cellwise[i]*tempU.coeffs[ i ].second;
		CqGuL_global.block(i,0,2,1) += CqGuL;
	} 
	...
	for ( unsigned int i=0; i < nCells; i++ )
	{
		...
		// length = k+1
		res1.coeffs[ i ].second = system->A_cellwise[i]*tempQ.coeffs[ i ].second - system->B_cellwise[i].transpose()*tempU.coeffs[ i ].second + system->C_cellwise[i].transpose()*lamCell - system->RF_cellwise[i].block(0,0,k+1,1);

		//Res2 = B*Q + D*U + X*Udot + E*Lam - F		
		// length = k+1
		res2.coeffs[ i ].second = system->B_cellwise[i]*tempQ.coeffs[ i ].second + system->D_cellwise[i]*tempU.coeffs[ i ].second + system->E_cellwise[i]*lamCell - system->RF_cellwise[ i ].block( k + 1, 0, k + 1, 1 ) + tempdudt.coeffs[ i ].second;
	} 
