...
SystemSolver::SystemSolver(Grid const &Grid, unsigned int polyNum,
                           TransportSystem *transpSystem)
    : grid(Grid), k(polyNum), nCells(Grid.getNCells()),
      nVars(transpSystem->getNumVars()),
      nScalars(transpSystem->getNumScalars()), nAux(transpSystem->getNumAux()),
      MXSolvers(Grid.getNCells()), y(nVars, grid, k, nScalars, nAux),
      dydt(nVars, grid, k, nScalars, nAux),
      yJac(nVars, grid, k, nScalars, nAux),
      dydtJac(nVars, grid, k, nScalars, nAux), problem(transpSystem) {
...
void SystemSolver::setInitialConditions(N_Vector &Y, N_Vector &dYdt) {
  ...
  resetCoeffs();
  if (!initialised)
    throw std::logic_error("setInitialConditions can only be called after "
  ...
  if (problem->isRestarting()) {
    ...
    auto initial_q =
        std::bind_front(&TransportSystem::InitialDerivative, problem);
    y.AssignU(initial_u);
    y.AssignQ(initial_q);
    ...
    if (nAux > 0) {
      auto initial_aux =
          std::bind_front(&TransportSystem::InitialAuxValue, problem);
...
void SystemSolver::initialiseMatrices() {
  ...
  // Temporary per-variable matrices that will be assembled into the larger cell
  // matrices as blocks
  ...
  for (unsigned int i = 0; i < nCells; i++) {
    ...
    for (Index var = 0; var < nVars; var++) {
      ...
      for (Eigen::Index i = 0; i < k + 1; i++) {
        for (Eigen::Index j = 0; j < k + 1; j++) {
          Dvar(i, j) += tau(I.x_l) * y.getBasis().Evaluate(I, j, I.x_l) *
                            y.getBasis().Evaluate(I, i, I.x_l) +
                        tau(I.x_u) * y.getBasis().Evaluate(I, j, I.x_u) *
                            y.getBasis().Evaluate(I, i, I.x_u);
        }
      }
    ...
    M.block(0, nVars * (k + 1), nVars * (k + 1), nVars * (k + 1))
        .setZero(); // NLq added at Jac step
    M.block(0, 2 * nVars * (k + 1), nVars * (k + 1), nVars * (k + 1))
        .setZero(); // NLu added at Jac step

    // row2
    M.block(nVars * (k + 1), 0, nVars * (k + 1), nVars * (k + 1)).setZero();
    M.block(nVars * (k + 1), nVars * (k + 1), nVars * (k + 1),
            nVars * (k + 1)) = -A;
    M.block(nVars * (k + 1), 2 * nVars * (k + 1), nVars * (k + 1),
            nVars * (k + 1)) = -B.transpose();

    // row3
    M.block(2 * nVars * (k + 1), 0, nVars * (k + 1), nVars * (k + 1)) = B;
    M.block(2 * nVars * (k + 1), nVars * (k + 1), nVars * (k + 1),
            nVars * (k + 1))
        .setZero();
    M.block(2 * nVars * (k + 1), 2 * nVars * (k + 1), nVars * (k + 1),
            nVars * (k + 1)) = D; // X added at Jac step

    // TODO:  Consider factorization here (is M sparse nough to warrant a sparse
    // implementation?)
    ...
    for (Index var = 0; var < nVars; var++) {
      ...
      for (Index i = 0; i < k + 1; i++) {
        ...
        if (I.x_l == grid.lowerBoundary() &&
            problem->isLowerBoundaryDirichlet(var)) {
          Cvar(0, i) = 0;
          Evar(i, 0) = 0;
        }
        // should this be is upper boundary dirichlet?
        if (I.x_u == grid.upperBoundary() &&
            problem->isUpperBoundaryDirichlet(var)) {
          Cvar(1, i) = 0;
          Evar(i, 1) = 0;
        }
      }
    ...
    CE_vec.block(nVars * (k + 1), 0, nVars * (k + 1), nVars * 2) =
        C.transpose();
    ...
    for (Index var = 0; var < nVars; var++) {
      if (I.x_l == grid.lowerBoundary() &&
        ...
        for (Eigen::Index j = 0; j < k + 1; j++) {
          ...
          RF_cellwise[i](j + var * (k + 1)) +=
              -y.getBasis().Evaluate(I, j, I.x_l) * (-1) *
              problem->LowerBoundary(var, 0.0);
          // < ( tau ) g_D, w >
          RF_cellwise[i](nVars * (k + 1) + j + var * (k + 1)) +=
              y.getBasis().Evaluate(I, j, I.x_l) * tau(I.x_l) *
              problem->LowerBoundary(var, 0.0);
        }
      }
      ...
      if (I.x_u == grid.upperBoundary() &&
        ...
        for (Eigen::Index j = 0; j < k + 1; j++) {
          ...
          RF_cellwise[i](j + var * (k + 1)) +=
              -y.getBasis().Evaluate(I, j, I.x_u) * (+1) *
              problem->UpperBoundary(var, 0.0);
          RF_cellwise[i](nVars * (k + 1) + j + var * (k + 1)) +=
              y.getBasis().Evaluate(I, j, I.x_u) * tau(I.x_u) *
              problem->UpperBoundary(var, 0.0);
        }
      }
    }
    ...
    // If (boundary cell)
    // C is multiplied by Q and not sigma, so we need to change the structure of
    // CG cellwise, also need to zero the contribution of sigma at the boundary
    // so it can float if the flux is a complicated nonlinear function of the
    // other variables, zero sigma does not necessarily mean zero q So we set up
    // a "double" C matrix that includes all of this information and set the
    // sigma and q portions at once
    ...
    for (Index var = 0; var < nVars; var++) {
      ...
      for (Index i = 0; i < k + 1; i++) {
        ...
        // If we have neumann boundaries, need to also set the boundary
        // parameters for q
        if (I.x_l == grid.lowerBoundary() &&
            !problem->isLowerBoundaryDirichlet(var)) {
          if (zeroFlux) {
            Csigma_var(0, i) =
                -y.getBasis().Evaluate(I, i, I.x_l); // Treat sigma as neumann
            Cq_var(0, i) = 0.0;
          } else {
            Csigma_var(0, i) = 0.0; // Treat sigma as dirichlet
            Cq_var(0, i) =
                -y.getBasis().Evaluate(I, i, I.x_l); // Treat Q as neumann
          }
        }
        if (I.x_u == grid.upperBoundary() &&
            !problem->isUpperBoundaryDirichlet(var)) {
          if (zeroFlux) {
            Csigma_var(1, i) =
                y.getBasis().Evaluate(I, i, I.x_u); // Treat sigma as neumann
            Cq_var(1, i) = 0.0;
          } else {
            Csigma_var(1, i) = 0.0; // Treat sigma as dirichlet
            Cq_var(1, i) =
                y.getBasis().Evaluate(I, i, I.x_u); // Treat q as neumann
          }
        }
    ...
    CG_cellwise[i].block(0, 2 * nVars * (k + 1), 2 * nVars, nVars * (k + 1)) =
        G; // this is the U block
    CG_cellwise[i].block(0, 0, 2 * nVars, nVars * (k + 1)) =
        Csigma; // This is the sigma block
    CG_cellwise[i].block(0, nVars * (k + 1), 2 * nVars, nVars * (k + 1)) =
        Cq; // This is the q block
    ...
    for (Index var = 0; var < nVars; var++) {
      if (I.x_l == grid.lowerBoundary() &&
          /* is b.d. Neumann at lower boundary */ !problem
              ->isLowerBoundaryDirichlet(var))
        L_global(var * (nCells + 1) + i) += -problem->LowerBoundary(var, 0.0);
      if (I.x_u == grid.upperBoundary() &&
          /* is b.d. Neumann at upper boundary */ !problem
              ->isUpperBoundaryDirichlet(var))
        L_global(var * (nCells + 1) + i + 1) +=
            problem->UpperBoundary(var, 0.0);
    }
    ...
    for (Index var = 0; var < nVars; var++) {
      ...
      y.getBasis().MassMatrix(
          I, Xvar, [this, var](double x) { return problem->aFn(var, x); });
...
// Memory Layout for a sundials Y is, if i indexes the components of u / q /
// sigma Y = [ sigma[ cell0, i=0 ], ..., sigma[ cell0, i= nVars - 1], q[ cell0,
// i = 0 ], ..., q[ cell0, i = nVars-1 ], u[ cell0, i = 0 ], .. u[ cell0, i =
// nVars - 1], sigma[ cell1, i=0 ], .... , u[ cellN-1, i = nVars - 1 ], Lambda[
// cell0, i=0 ],.. ]
//
...
void SystemSolver::updateMatricesForJacSolve() {
  ...
  for (unsigned int i = 0; i < nCells; i++) {
    ...
    MX.block(2 * nVars * (k + 1), nVars * (k + 1), nVars * (k + 1),
             nVars * (k + 1)) -= Sq;
...
void SystemSolver::setJacEvalY(N_Vector yy, N_Vector yp) {
  ...
  yJac.copy(yyMap); // Deep copy -- yyMap only aliases the N_Vector, this copies
                    // the data
...
// Over-arching Jacobian function. If there's no coupled B-field solve, or
// auxiliar variables, then just do the HDG Jacobian solve
void SystemSolver::solveJacEq(N_Vector res_g, N_Vector delY) {
  if (nScalars > 0) {
    // TODO: move temporaries into private variables of the class and
    // allocate/destroy once allocate temporary working space for gauss
    // elimination of scalars.
    ...
    N_VLinearCombination(nScalars, tmp_N.data(), e,
                         g);         // g = Sum_i tmp_N[i]*e[i]
    ...
    Vector NweInv_w_g = -1 * Nwe.inverse() *
                        wDotg; // Uses PartialPivLU internally, never really
                               // does inverse (except for small matrices)
    N_VLinearCombination(nScalars, NweInv_w_g.data(), e,
                         delY); // Set delY = - [ e  ( N + w^T e )^-1  w ]  g
    N_VLinearSum(
        1.0, delY, 1.0, g,
        delY); // delY += g; so delY = g - (....), which is the final answer
...
void SystemSolver::solveHDGJac(N_Vector g, N_Vector delY) {
  ...
  for (Index i = 0; i < nCells; i++) {
    ...
    for (Index varI = 0; varI < nVars; varI++)
      for (Index varJ = 0; varJ < nVars; varJ++)
        K_global.block<2, 2>(varI * (nCells + 1) + i,
                             varJ * (nCells + 1) + i) +=
  ...
  for (Index i = 0; i < nCells; i++) {
    for (Index var = 0; var < nVars; var++) {
      F.block<2, 1>(var * (nCells + 1) + i, 0) -=
          (CG_cellwise[i] * SQU_f[i]).block(var * 2, 0, 2, 1);
    }
  }

  // Factorise the global matrix ( size n_cells * n_variables )
  EigenGlobalSolver globalKSolver(K_global);
  // This solves for the lambdas of all variables at once (drop it in the memory
  // sundials reserved for it)
  ...
  // If the BCs are Dirichlet, enforce that (Y + delY).lambda( v )[0,N] are the
  right values for ( Index i=0; i < nVars; i++ ) { if (
  problem->isLowerBoundaryDirichlet( i ) ) del_y.lambda( i )[ 0 ] =
  problem->LowerBoundary( i, t ) - y.lambda( i )[ 0 ]; if (
  problem->isUpperBoundaryDirichlet( i ) ) del_y.lambda( i )[ nCells ] =
  problem->UpperBoundary( i, t ) - y.lambda( i )[ nCells ];
  }
  */
  ...
  for (Index i = 0; i < nCells; i++) {
    ...
    for (Index var = 0; var < nVars; var++) {
      delLambdaCell.block<2, 1>(2 * var, 0) =
          delYVec.segment(LambdaOffset + var * (nCells + 1) + i, 2);
    }

    /*
    // Try mapping the memory by using the magic runes (future update)
    Eigen::Map< Vector, 0, Eigen::InnerStride<nCells + 1> >
    delLambdaCell( delYVec.data() + LambdaOffset + i, 2 * nVars,
    Eigen::InnerStride<nCells + 1> );
    */

    delSQU = SQU_f[i] - SQU_0[i] * delLambdaCell;
    for (Index var = 0; var < nVars; var++) {
      del_y.sigma(var).getCoeff(i).second = delSQU.segment(var * S_DOF, S_DOF);
      del_y.q(var).getCoeff(i).second =
          delSQU.segment(nVars * S_DOF + var * Q_DOF, Q_DOF);
      del_y.u(var).getCoeff(i).second =
          delSQU.segment(nVars * (S_DOF + Q_DOF) + var * U_DOF, U_DOF);
    }
    for (Index aux = 0; aux < nAux; aux++)
      del_y.Aux(aux).getCoeff(i).second =
          delSQU.segment(nVars * SQU_DOF + aux * AUX_DOF, AUX_DOF);
  }
}

int static_residual(sunrealtype tres, N_Vector Y, N_Vector dYdt,
                    N_Vector resval, void *user_data) {
  auto system = static_cast<SystemSolver *>(user_data);
  try {
    return system->residual(tres, Y, dYdt, resval);
  } catch (std::exception &e) {
    std::cout << "Caught exception : " << e.what() << " ; Retrying. "
              << std::endl;
    return 1;
  }
}
...
int SystemSolver::residual(sunrealtype tres, N_Vector Y, N_Vector dYdt,
  ...
  for (Index i = 0; i < nCells; i++) {
    ...
    for (Index var = 0; var < nVars; var++) {
      res.lambda(var).segment<2>(i) +=
          Csigma_cellwise[i].block(var * 2, var * (k + 1), 2, k + 1) *
              Y_h.sigma(var).getCoeff(i).second +
          Cq_cellwise[i].block(var * 2, var * (k + 1), 2, k + 1) *
              Y_h.q(var).getCoeff(i).second +
          G_cellwise[i].block(var * 2, var * (k + 1), 2, k + 1) *
              Y_h.u(var).getCoeff(i).second +
          H_cellwise[i].block(2 * var, 2 * var, 2, 2) *
              Y_h.lambda(var).segment<2>(i) -
          L_global.segment<2>(var * (nCells + 1) + i);
    }
  }
  ...
  for (Index i = 0; i < nCells; i++) {
    ...
    for (Index var = 0; var < nVars; var++) {
      ...
      Eigen::VectorXd kappa_cellwise =
          y.getBasis().InterpolateOntoBasis(I, Sigma_vals[var](ind));

      // Evaluate Source Function
      Eigen::VectorXd S_cellwise =
          y.getBasis().InterpolateOntoBasis(I, Source_vals[var](ind));

      auto const &lambda = lamCell.segment<2>(2 * var);

      // We should normalise the components of the residual such that the
      // `sigma' component of res has tolerances that are the same as `sigma'
      // itself.
      //
      // For sigma and q, just make the 'sigma-determinitive' equation the sigma
      // component and the same for q. as these equations are proportional to
      // the variables themselves, we are done

      res.sigma(var).getCoeff(i).second =
          A_cellwise[i].block(var * (k + 1), var * (k + 1), k + 1, k + 1) *
              Y_h.sigma(var).getCoeff(i).second +
          kappa_cellwise;
      ...
      res.q(var).getCoeff(i).second =
          -A_cellwise[i].block(var * (k + 1), var * (k + 1), k + 1, k + 1) *
              Y_h.q(var).getCoeff(i).second -
          B_cellwise[i].transpose().block(var * (k + 1), var * (k + 1), k + 1,
                                          k + 1) *
              Y_h.u(var).getCoeff(i).second +
          C_cellwise[i].transpose().block(var * (k + 1), var * 2, k + 1, 2) *
              lambda -
...
void SystemSolver::initializeMatricesForAdjointSolve() {
  ...
  for (unsigned int i = 0; i < nCells; i++) {
    ...
    // row3
    M.block(2 * nVars * (k + 1), 0, nVars * (k + 1), nVars * (k + 1)) =
        B - Ssig;
    M.block(2 * nVars * (k + 1), nVars * (k + 1), nVars * (k + 1),
            nVars * (k + 1)) = Sq;
    M.block(2 * nVars * (k + 1), 2 * nVars * (k + 1), nVars * (k + 1),
    ...
    CG_vec.block(0, nVars * (k + 1), 2 * nVars, nVars * (k + 1)) =
        Cq_cellwise[i];
...
void SystemSolver::solveAdjointState(Index gIndex) {
  ...
  for (Index i = 0; i < nCells; i++) {
    for (Index var = 0; var < nVars; var++) {
      F.block<2, 1>(var * (nCells + 1) + i, 0) -=
          (adjoint_CEBlocks[i] * SQU_f[i]).block(var * 2, 0, 2, 1);
    }
  }
  ...
  */
  ...
  for (Index i = 0; i < nCells; i++) {
    ...
    for (Index var = 0; var < nVars; var++) {
      LambdaCell.block<2, 1>(2 * var, 0) =
          adjoint_lambdas.segment(var * (nCells + 1) + i, 2);
    }
...
void SystemSolver::computeAdjointGradients() {
  ...
  for (Index i = 0; i < adjointProblem->getNg(); i++) {
    if (adjointProblem->areParametersSpatial()) {
      checkShapeAndSet(G_p.block(i * nCells * (k + 1), 0, nCells * (k + 1),
                                 adjointProblem->getNp()),
                       adjointProblem->dGFndp(i, y), "dGdp in SystemSolver");
    } else
      G_p.row(i) = adjointProblem->dGFndp(i, y);
  }

  // Index np = adjointProblem->areParametersSpatial() ? np_internal * nCells *
  // (k + 1) + adjointProblem->getNpBoundary() : adjointProblem->getNp();
    ...
    for (Index i = 0; i < nCells; ++i) {
      ...
      for (Index var = 0; var < nVars; ++var) {
        ...
        if (adjointProblem->areParametersSpatial()) {
          for (Index j = 0; j < k + 1; j++) {
            ...
            F_p.block(var * (k + 1) + 2 * nVars * (k + 1), j, k + 1, 1) =
                -dSdp_cellwise;
          }
        } else {
          dkappa_dp_phi.setZero();
          if (adjointProblem->isAdjointIndexInternal(pIndex)) {
            const auto dSigmadp_cell = dSigmadp.Variable(i)[var];
            dkappa_dp_phi = y.getBasis().InterpolateOntoBasis(
                I, dSigmadp_cell(pIndex, Eigen::all));
          }
          ...
          if (adjointProblem->isAdjointIndexInternal(pIndex)) {
            const auto dSourcedp_cell = dSourcedp.Variable(i)[var];
            dSdp_cellwise = y.getBasis().InterpolateOntoBasis(
                I, dSourcedp_cell(pIndex, Eigen::all));
          }

          F_p.block(var * (k + 1), 0, k + 1, 1) = dkappa_dp_phi;

          // auto C_cell = C_cellwise[i];
          F_p.block(var * (k + 1) + 2 * nVars * (k + 1), 0, k + 1, 1) =
              -dSdp_cellwise;
        }
        ...
        if (I.x_l == grid.lowerBoundary() &&
            adjointProblem->computeLowerBoundarySensitivity(var, pIndex)) {
          for (Eigen::Index j = 0; j < k + 1; j++) {
            F_p(nVars * (k + 1) + j + var * (k + 1)) +=
                y.getBasis().Evaluate(I, j, I.x_l);
          }
        }

        if (I.x_u == grid.upperBoundary() &&
            adjointProblem->computeUpperBoundarySensitivity(var, pIndex)) {
          for (Eigen::Index j = 0; j < k + 1; j++) {
            // < g_D , v . n > ~= g_D( x_1 ) * phi_j( x_1 ) * ( n_x = +1 )
            F_p(nVars * (k + 1) + j + var * (k + 1)) +=
                y.getBasis().Evaluate(I, j, I.x_u);
          }
        }
        // TODO: implement this
      }
      for (Index aux = 0; aux < nAux; ++aux) {
        if (adjointProblem->isAdjointIndexInternal(pIndex)) {
          ...
          Eigen::VectorXd dAux_dp_cellwise =
              y.getBasis().ProjectOntoBasis(I, dAuxdp);
          F_p.block(3 * nVars * (k + 1) + aux * (k + 1), 0, k + 1, 1) =
              dAux_dp_cellwise;
        }
      }
      ...
      for (Index gIndex = 0; gIndex < adjointProblem->getNg(); gIndex++) {
        if (adjointProblem->areParametersSpatial()) {
          ...
          G_p(gIndex, pIndex) -=
              adjoint_squ[i].transpose() * static_cast<Vector>(F_p);
      }

      // Eigen::VectorXd dkappa_lambda = C_cell * dkappa_dp_phi;
      // // // // // Lambda portion
      // G_p(pIndex) -= adjoint_lambdas.segment(i, 2).transpose() *
      // dkappa_lambda;
    }
  }
}
...
void SystemSolver::print(std::ostream &out, double t, int nOut,
                         N_Vector const &tempY, bool printSources) {
...
void SystemSolver::printOnNodes(std::ostream &out, double t,
                                N_Vector const &tempY, bool printSources) {
  ...
  if (printSources) {
    for (Index v = 0; v < nVars; ++v) {
      sources[v] =
          problem->Sources(v, tmp_y.evalOnNodes(), tmp_y.getPoints(), t);
    }
  }
...
int SystemSolver::getErrorWeights(N_Vector y_sundials, N_Vector ewt_sundials) {
  ...
  for (Index i = 0; i < nCells; ++i) {
    ...
    for (Index v = 0; v < nVars; ++v) {
      ...
      ewt.u(v).getCoeff(i).second =
          1.0 / (rtol * abs(y.u(v).getCoeff(i).second.array()) + absTol);
      ewt.q(v).getCoeff(i).second =
          1.0 / (rtol * abs(y.q(v).getCoeff(i).second.array()) + absTol);
      ewt.sigma(v).getCoeff(i).second =
          1.0 / (rtol * abs(y.sigma(v).getCoeff(i).second.array()) + absTol);
    }

    for (Index a = 0; a < nAux; ++a) {

      ewt.Aux(a).getCoeff(i).second =
          1.0 / (rtol * abs(y.Aux(a).getCoeff(i).second.array()) + absTol);
    }
  }
  ...
  for (Index i = 0; i < nScalars; ++i) {
    ...
    ewt.Scalar(i) =
        ::sqrt(localDOF * nCells) / (rtol * abs(y.Scalar(i)) + absTol);
  }
...
void SystemSolver::PrintDebugInfo() {
  ...
  for (Index i = 0; i < nCells; i++) {
    ...
    std::cout << MBlocks[i] << std::endl << std::endl;
  }
}
