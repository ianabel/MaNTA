...
void CMFXparallellosses::pickVariables()
{
	addVariable("P_ion");
	addVariable("P_e");
	addVariable("omega");

	mirrorPlasma = std::make_unique<MirrorPlasma>();
}

void CMFXparallellosses::seta_fns()
{
...
void CMFXparallellosses::setKappas()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");
...
void CMFXparallellosses::setSources()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");
	...
	std::function<double( double, DGApprox, DGApprox, DGApprox )> sourceP_ion = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		//std::cerr << mirrorPlasma->ParallelIonHeatLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R) << std::endl;
		return -3.0/(10.0*Om_i*Om_i)*R*R*R*u(R,P_ion.index)*q(R,omega.index)*q(R,omega.index)/tauI(u(R,P_ion.index),R) 
			- R*Ci(u(R,P_ion.index), u(R,P_e.index), R)
			+R*mirrorPlasma->ParallelIonHeatLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R);};
	
	std::function<double( double, DGApprox, DGApprox, DGApprox )> sourceP_e = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return -R*Ce(u(R,P_ion.index), u(R,P_e.index), R)
			+R*mirrorPlasma->ParallelElectronHeatLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R, omega.index), R);};
	
	std::function<double( double, DGApprox, DGApprox, DGApprox )> sourceOmega = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return -I_r(R)*R*R*B_mid
		+R*mirrorPlasma->ParallelIonParticleLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R)*mirrorPlasma->ionSpecies.Mass*protonMass*u(R, omega.index);};
...
void CMFXparallellosses::setdudKappas()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");
...
void CMFXparallellosses::setdqdKappas()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");
...
void CMFXparallellosses::setdudSources()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");
	...
	//----------------P_ion----------------------
	std::function<double( double, DGApprox, DGApprox, DGApprox )> dS_PidPi = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return -3.0/(10.0*Om_i*Om_i)*R*R*R*q(R,omega.index)*q(R,omega.index)/tauI(u(R,P_ion.index),R) + 3.0/(10.0*Om_i*Om_i)*R*R*R*u(R,P_ion.index)*q(R,omega.index)*q(R,omega.index)/(tauI(u(R,P_ion.index),R)*tauI(u(R,P_ion.index),R))*dtauIdP_i(u(R,P_ion.index),R)
		- R*dCidPi(u(R,P_ion.index),u(R,P_e.index),R)
		+(R/n(R))*mirrorPlasma->delTi_ParallelIonHeatLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R) ;};
	
	std::function<double( double, DGApprox, DGApprox, DGApprox )> dS_PidPe = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return -R*dCidPe(u(R,P_ion.index),u(R,P_e.index),R)
		+(R/n(R))*mirrorPlasma->delTe_ParallelIonHeatLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R);};

	std::function<double( double, DGApprox, DGApprox, DGApprox )> dS_Pidomega = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return R*mirrorPlasma->delOmega_ParallelIonHeatLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R);};
	...
	//----------------P_e----------------------
	std::function<double( double, DGApprox, DGApprox, DGApprox )> dS_PedPi = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return - R*dCedPi(u(R,P_ion.index),u(R,P_e.index),R)
		+(R/n(R))*mirrorPlasma->delTi_ParallelElectronHeatLoss();};

	std::function<double( double, DGApprox, DGApprox, DGApprox )> dS_PedPe = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return -R*dCedPe(u(R,P_ion.index),u(R,P_e.index),R)
		+(R/n(R))*mirrorPlasma->delTe_ParallelElectronHeatLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R);};

	std::function<double( double, DGApprox, DGApprox, DGApprox )> dS_Pedomega = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return +R*mirrorPlasma->delOmega_ParallelElectronHeatLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R);};
	...
	//----------------omega----------------------
	std::function<double( double, DGApprox, DGApprox, DGApprox )> dS_omegadPi = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return +R*mirrorPlasma->ionSpecies.Mass*protonMass*u(R, omega.index)/n(R)*mirrorPlasma->delTi_ParallelIonParticleLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R);};
	std::function<double( double, DGApprox, DGApprox, DGApprox )> dS_omegadPe = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return +R*mirrorPlasma->ionSpecies.Mass*protonMass*u(R, omega.index)/n(R)*mirrorPlasma->delTe_ParallelIonParticleLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R);};
	std::function<double( double, DGApprox, DGApprox, DGApprox )> dS_omegadomega = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){
		return +R*mirrorPlasma->ionSpecies.Mass*protonMass*(mirrorPlasma->ParallelIonParticleLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R) + u(R,omega.index)*mirrorPlasma->delOmega_ParallelIonParticleLoss(u(R,P_e.index)/n(R), u(R,P_ion.index)/n(R), u(R,omega.index), R));};
...
void CMFXparallellosses::setdqdSources()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");
...
void CMFXparallellosses::setdsigdSources()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");
...
double CMFXparallellosses::tauI(double Pi, double R)
{
...
double CMFXparallellosses::dtauIdP_i(double Pi, double R)
{
...
double CMFXparallellosses::lambda(double R)
{
	return 15.0;
...
double CMFXparallellosses::tauE(double Pe, double R)
{
...
double CMFXparallellosses::dtauEdP_e(double Pe, double R)
{
...
double CMFXparallellosses::nu(double Pi, double Pe, double R)
{
...
double CMFXparallellosses::dnudPi(double Pi, double Pe,  double R)
{
	//double alpha = ::pow(e_charge*e_charge/(2*M_PI*eps_0*me),2)*n(R)*lambda(R);
...
double CMFXparallellosses::dnudPe(double Pi, double Pe,  double R)
{
	//double alpha = ::pow(e_charge*e_charge/(2*M_PI*eps_0*me),2)*n(R)*lambda(R);
...
double CMFXparallellosses::Ce(double Pi, double Pe, double R)
{
	return nu(Pi, Pe, R)*(Pi-Pe)/n(R);
}

double CMFXparallellosses::dCedPe(double Pi, double Pe, double R)
{
	return -nu(Pi,Pe,R)/n(R) + dnudPe(Pi,Pe,R)*(Pi-Pe)/n(R);
}

double CMFXparallellosses::dCedPi(double Pi, double Pe, double R)
{
	return nu(Pi,Pe,R)/n(R) + dnudPi(Pi,Pe,R)*(Pi-Pe)/n(R);
}

double CMFXparallellosses::Ci(double Pi, double Pe, double R)
{
	return -Ce(Pi, Pe, R);
}

double CMFXparallellosses::dCidPe(double Pi, double Pe, double R)
{
	return -dCedPe(Pi, Pe, R);
}

double CMFXparallellosses::dCidPi(double Pi, double Pe, double R)
{
	return -dCedPi(Pi, Pe, R);
}

double CMFXparallellosses::I_r(double R)
{
	return 0.0e-2;
}