#include "MirrorModel.hpp"
#include "../Constants.hpp"

#include <math.h>

void MirrorModel::pickVariables()
{
	addVariable("P_ion");
	addVariable("P_e");
	addVariable("omega");
...
double MirrorModel::IonViscousHeating( double R, double dOmegadPsi, double Pi ) 
{
  return -3.0/(10.0*Om_i*Om_i)*R*R*R * Pi * dOmegadPsi * dOmegadPsi / tauI(Pi,R);
}
void MirrorModel::setSources()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");

	DGFunction sourceP_ion = [ = ]( double R, DGApprox sig, DGApprox q, DGApprox u ){ 
		double dOmegadPsi = q( R,omega.index );
		double 
		return - R*Ci(u(R,P_ion.index), u(R,P_e.index), R);
	};
...
void MirrorModel::setdudSources()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");
...
void MirrorModel::setdqdSources()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");
...
void MirrorModel::setdsigdSources()
{
	auto& P_ion = variables.at("P_ion");
	auto& P_e = variables.at("P_e");
	auto& omega = variables.at("omega");
...
// Ion-ion collision frequency in SI units
double MirrorModel::tauI(double Pi, double n)
{
 	if(Pi>0)
		T = Pi/n;
	else  //if we have a negative temp just treat it as 1eV
		T = 1.0;
	return 3.44e11*(1.0/::pow(n,5.0/2.0))*(::pow(J_eV(Pi),3.0/2.0))*(1.0/lambda(n,T))*(::sqrt(mi/me));
}

double MirrorModel::dtauIdP_i(double Pi, double R)
{
...
double MirrorModel::lambda(double n, double T)
{
	return 15.0;
...
double MirrorModel::tauE(double Pe, double R)
{
...
double MirrorModel::dtauEdP_e(double Pe, double R)
{
...
double MirrorModel::nu(double Pi, double Pe, double R)
{
...
double MirrorModel::dnudPi(double Pi, double Pe,  double R)
{
	//double alpha = ::pow(e_charge*e_charge/(2*M_PI*eps_0*me),2)*n(R)*lambda(R);
...
double MirrorModel::dnudPe(double Pi, double Pe,  double R)
{
	//double alpha = ::pow(e_charge*e_charge/(2*M_PI*eps_0*me),2)*n(R)*lambda(R);
...
double MirrorModel::Ce(double Pi, double Pe, double R)
{
	return nu(Pi, Pe, R)*(Pi-Pe)/n(R);
}

double MirrorModel::dCedPe(double Pi, double Pe, double R)
{
	return -nu(Pi,Pe,R)/n(R) + dnudPe(Pi,Pe,R)*(Pi-Pe)/n(R);
}

double MirrorModel::dCedPi(double Pi, double Pe, double R)
{
	return nu(Pi,Pe,R)/n(R) + dnudPi(Pi,Pe,R)*(Pi-Pe)/n(R);
}

double MirrorModel::Ci(double Pi, double Pe, double R)
{
	return -Ce(Pi, Pe, R);
}

double MirrorModel::dCidPe(double Pi, double Pe, double R)
{
	return -dCedPe(Pi, Pe, R);
}

double MirrorModel::dCidPi(double Pi, double Pe, double R)
{
	return -dCedPi(Pi, Pe, R);
}

double MirrorModel::L_i(double R) const
{
	return mi*n(R)*B_mid*10e8;
}

double MirrorModel::Jr(DGApprox sigma, double R) const
{
	auto& omega = variables.at("omega");
	double Jr = 0.0;
...
double MirrorModel::EvalCoeffs( LegendreBasis & B, Coeff_t cs, double x, int var ) const
{
