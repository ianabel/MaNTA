...
class JAXNonlinearDiffusion(VectorizedTransportSystem):
    def __init__(self, config):
        # Two variables, so `sigma` below has to index by `index` rather than
        # reach for [0] -- which is the point of driving the batched interface.
        super().__init__(MaNTA.numbered_spec(2, lower=MaNTA.Neumann))
