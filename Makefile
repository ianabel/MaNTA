...
SOURCES = Config.cpp ConfigSchema.cpp SolverConfig.cpp SystemSolver.cpp SunLinSolWrapper.cpp SunMatrixWrapper.cpp ErrorChecker.cpp Solver.cpp Matrices.cpp DGStatic.cpp PhysicsCases.cpp NetCDFIO.cpp AdjointVectors.cpp Postprocessing.cpp
...
# one, and the DESC equilibria in python-examples/stellarator/desc_optimize.py
# are expensive.
#
# `python` itself is deliberately absent: since the drivers moved to
# python-examples/ that directory holds only the package and its tests, and no
# run writes output there.
