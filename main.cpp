...
int main()
{
	...
	int nOut = 300;
	double tFinal = 1.0, delta_t = 0.1;
	realtype rtol = 1.0e-3, atol = 1.0e-3;
	...
	std::function<double( double )> sigma_0 = [=]( double y ){ return 1.0 + beta*u_0(y)*u_0(y)*gradu_0(y); };
...
void buildDiffusionObj(std::shared_ptr<DiffusionObj> diffobj)
{
	...
	std::function<double( double, DGApprox, DGApprox )> kappa0 = [ = ]( double x, DGApprox q, DGApprox u ){ return  1.0 + beta*u(x,0)*u(x,0)*q(x,0);};
	diffobj->kappaFuncs.push_back(kappa0);

	std::function<double( double, DGApprox, DGApprox )> dkappa0dq0 = [ = ]( double x, DGApprox q, DGApprox u ){ return 2*beta*u(x,0)*q(x,0);};
	//std::function<double( double, DGApprox, DGApprox )> dkappa0dq0 = [ = ]( double x, DGApprox q, DGApprox u ){ return 2*beta;};

	std::function<double( double, DGApprox, DGApprox )> dkappa0du0 = [ = ]( double x, DGApprox q, DGApprox u ){ return beta*u(x,0)*u(x,0);};
	//std::function<double( double, DGApprox, DGApprox )> dkappa0du0 = [ = ]( double x, DGApprox q, DGApprox u ){ return 0.0;};
