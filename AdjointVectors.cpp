...
void SystemSolver::DerivativeSubVector(Index gIndex, Vector &Vec,
                                       Eigen::Ref<Matrix> const dX_dZ,
                                       DGSoln const &Y, Index intervalIndex) {
  ...
  for (Index XVar = 0; XVar < nVars; XVar++) {
    ...
    Vec.block(XVar * (k + 1), 0, (k + 1), 1) =
        Y.getBasis().InterpolateOntoBasis(I, dX_dZ_vec);
  }
}

void SystemSolver::DerivativeSubVector(
    Index gIndex, Vector &Vec,
    void (AdjointProblem::*dX_dZ)(Index, VectorRef, const State &, Position),
    DGSoln const &Y, Index intervalIndex) {
  Interval const &I(grid[intervalIndex]);
  ...
  // Phi are basis fn's
  // M( nVars * K + k, nVars * J + j ) = Int_I ( d sigma_fn_K / d u_J * Phi_k *
  // Phi_j )
...
void SystemSolver::dGdu_Vec(Index gIndex, Vector &Vec, DGSoln const &Y,
                            Index I) {
  DerivativeSubVector(gIndex, Vec, &AdjointProblem::dgFn_du, Y, I);
}

void SystemSolver::dGdq_Vec(Index gIndex, Vector &Vec, DGSoln const &Y,
                            Index I) {
  DerivativeSubVector(gIndex, Vec, &AdjointProblem::dgFn_dq, Y, I);
}

void SystemSolver::dGdsigma_Vec(Index gIndex, Vector &Vec, DGSoln const &Y,
                                Index I) {
  DerivativeSubVector(gIndex, Vec, &AdjointProblem::dgFn_dsigma, Y, I);
}

void SystemSolver::dGdaux_Vec(Index gIndex, Vector &Vec, DGSoln const &Y,
                              Index intervalIndex) {
  Interval const &I(grid[intervalIndex]);
  auto const &x_vals = y.getBasis().abscissae();
  auto const &x_wgts = y.getBasis().weights();
...
// void SystemSolver::dSigmadp_Vec(Index i, Vector &Vec, DGSoln const &Y, Index
// I)
// {
//     DerivativeSubVector(i, Vec, &AdjointProblem::dSigmaFn_dp, Y, I);
// }
// void SystemSolver::dSourcesdp_Vec(Index i, Vector &Vec, DGSoln const &Y,
// Index I)
// {
