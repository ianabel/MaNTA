...
ThreeVarCylinder::ThreeVarCylinder(toml::value const &config, Grid const &grid)
    : AutodiffTransportSystem(config, grid, 3, 0, 0) // Configure a blank autodiff system with three variables and no scalars
{
...
Real ThreeVarCylinder::Source(Index i, RealVector u, RealVector q, RealVector sigma, RealVector, Position x, Time t)
{
    Channel c = static_cast<Channel>(i);
    switch (c)
    {
