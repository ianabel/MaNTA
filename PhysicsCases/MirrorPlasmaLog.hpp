#ifndef MIRRORPLASMALOG
#define MIRRORPLASMALOG
...
class MirrorPlasmaLog : public AutodiffTransportSystem
{
public:
    MirrorPlasmaLog(toml::value const &config, Grid const &grid);
    virtual ~MirrorPlasmaLog() { delete B; };
...
private:
    ...
    std::map<std::string, Index> ChannelMap = {{"Density", Channel::Density}, {"IonTemperature", Channel::IonTemperature}, {"ElectronTemperature", Channel::ElectronTemperature}, {"AngularMomentum", Channel::AngularMomentum}};
    std::map<Index, bool> ConstantChannelMap = {{Channel::Density, false}, {Channel::IonTemperature, false}, {Channel::ElectronTemperature, false}, {Channel::AngularMomentum, false}};
    ...
    REGISTER_PHYSICS_HEADER(MirrorPlasmaLog)
};
