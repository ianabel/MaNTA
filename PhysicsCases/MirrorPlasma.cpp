...
MirrorPlasma::MirrorPlasma(toml::value const &config, Grid const &grid) {
  ...
  if (config.count("MirrorPlasma") == 1) {
    ...
    lowerBoundaryConditions =
        toml::find_or(InternalConfig, "lowerBoundaryConditions",
                      std::vector<bool>(nVars, true));
    upperBoundaryConditions =
        toml::find_or(InternalConfig, "upperBoundaryConditions",
                      std::vector<bool>(nVars, true));
...
Real MirrorPlasma::qe(RealVector u, RealVector q, Real V, Time t) const {
  ...
  // Real rhoTe =
  //     abs(sqrt(Te) * Plasma->RhoStarRef() * Te_prime / B->dRdV(V, 0.0) / Te);
  // Real x = (rhoTe - lowPThreshold) / lowPThreshold;
  // if (x >= 0)
...
Real MirrorPlasma::Pi(RealVector u, RealVector q, Real V, Time t) const {
  ...
  Real rhoM =
      abs(sqrt(Ti) * Plasma->RhoStarRef() * dMdV / B->dRdV(V, 0.0) / M);
  Real x = (rhoM - lowLThreshold) / lowLThreshold;
  if (x >= 0)
    Pi_v += x * lowLDiffusivity * dMdV * GeometricFactor * R; //
