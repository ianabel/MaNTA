...
template <typename T> T sign(T x) { return x >= 0 ? 1 : -1; }
...
SystemSpec MirrorPlasma::buildSpec(toml::value const &config) {
  ...
  auto spec = SystemSpec{
      .variables = {{"Density", "particle density", "n0"},
                    {"IonEnergy", "ion energy density", "n0 T0"},
                    {"ElectronEnergy", "electron energy density", "n0 T0"},
                    {"AngularMomentum", "angular momentum density",
                     "n0 T0 / c_s0"}}};
  ...
  const auto lower = toml::find_or(InternalConfig, "lowerBoundaryConditions",
                                   std::vector<bool>(4, true));
  const auto upper = toml::find_or(InternalConfig, "upperBoundaryConditions",
                                   std::vector<bool>(4, true));
  for (Index i = 0; i < 4; ++i) {
    spec.variables[i].lower =
        lower[i] ? BoundaryKind::Dirichlet : BoundaryKind::Neumann;
    spec.variables[i].upper =
        upper[i] ? BoundaryKind::Dirichlet : BoundaryKind::Neumann;
  }

  if (toml::find_or(InternalConfig, "useAmbipolarPhi", false))
    spec.aux = {{"AmbipolarPhi",
  ...
  if (toml::find_or(InternalConfig, "useConstantVoltage", false))
    spec.scalars = {{"VoltageError", "V0 minus the achieved voltage", "", true},
                    {"VoltageErrorIntegral", "time integral of the error", "",
                     true},
...
MirrorPlasma::MirrorPlasma(toml::value const &config, Grid const &grid)
  ...
  if (config.count("MirrorPlasma") == 1) {
    ...
    std::vector<std::string> constProfiles = toml::find_or(
        InternalConfig, "ConstantProfiles", std::vector<std::string>());
    ...
    std::string Bfile =
        toml::find_or(InternalConfig, "MagneticFieldData", B_file);
    ...
    if (useNcBField)
      ...
      B = createMagneticField<StraightMagneticField>(
          L_z, B_z, Rm, xL, m); // std::make_shared<StraightMagneticField>(L_z,
                                // B_z, Rm, xL, m); //  //
    ...
    std::string IonType =
        toml::find_or(InternalConfig, "IonSpecies", "Deuterium");

    Plasma = std::make_unique<PlasmaConstants>(IonType, B, n0, T0, Z_eff,
                                               a * (R_Upper - R_Lower));
    ...
    useConstantVoltage =
        toml::find_or(InternalConfig, "useConstantVoltage", false);
    if (useConstantVoltage) {
      double cs0 = std::sqrt(T0 / Plasma->IonMass());
      V0 =
          toml::find_or(InternalConfig, "PlasmaVoltage", 50e3); // default 50 kV
      V0 /= cs0;
    ...
    growth_factors = toml::find_or(InternalConfig, "growth_factors",
                                   std::vector<double>(nVars, 1.0));
    growth_rate = toml::find_or(InternalConfig, "MMSgrowth_rate", 0.5);

    SourceCap = toml::find_or(InternalConfig, "SourceCap", 1e5);
    SourceRampup = toml::find_or(InternalConfig, "SourceRampup", 1e-3);

    loadInitialConditionsFromFile =
        toml::find_or(InternalConfig, "useNcFile", false);
    if (loadInitialConditionsFromFile) {
      filename = toml::find_or(InternalConfig, "InitialConditionFilename",
                               "MirrorPlasmaRERUN.nc");
    ...
    MLower = toml::find_or(InternalConfig, "LowerMachNumber",
                           R_Lower * omega_edge / sqrt(TeEdge));
    MUpper = toml::find_or(InternalConfig, "UpperMachNumber",
                           R_Upper * omega_edge / sqrt(TeEdge));
    MEdge = 0.5 * (MUpper + MLower);

    InitialPeakDensity = toml::find_or(InternalConfig, "InitialDensity", n_mid);
    InitialPeakTe =
        toml::find_or(InternalConfig, "InitialElectronTemperature", T_mid);
    InitialPeakTi =
        toml::find_or(InternalConfig, "InitialIonTemperature", T_mid);
    InitialPeakMachNumber =
        toml::find_or(InternalConfig, "InitialMachNumber", omega_mid);
    ...
    I0 = Plasma->IonMass() * n0 * sqrt(T0 / Plasma->IonMass()) * a * a * a /
         Plasma->NormalizingTime(); // Current normalizing factor
    IRadial /= I0;
    ...
    LowerParticleSourceStrength =
        toml::find_or(InternalConfig, "LowerPS", 10.0);

    UpperParticleSourceStrength = toml::find_or(InternalConfig, "UpperPS", 0.0);

    ParticleSourceWidth =
        toml::find_or(InternalConfig, "ParticleSourceWidth", 0.02);
    ParticleSourceCenter = toml::find_or(InternalConfig, "ParticleSourceCenter",
                                         0.5 * (R_Lower + R_Upper));
    ...
    throw std::invalid_argument(
        "To use the Mirror Plasma physics model, a [MirrorPlasma] "
        "configuration section is required.");
  } else {
    throw std::invalid_argument("Unable to find unique [MirrorPlasma] "
                                "configuration section in configuration file.");
  }
};
...
Real2nd MirrorPlasma::InitialFunction(Index i, Real2nd V, Real2nd t) const {
  ...
  Real2nd v = cos(pi * (R - R_mid) /
                  (R_max - R_min)); //* exp(-shape * (R - R_mid) * (R - R_mid));

  Real2nd Te = TeEdge + tfac(growth_factors[Channel::ElectronEnergy]) *
                            (TeMid - TeEdge) * v * v;
  // Real2nd omega = Omega_Lower + m * (V - xL) + OmegaMid * pow(cos(pi * (V -
  // xmid) / (xR - xL)), 1.0); TeEdge +
  // tfac(growth_factors[Channel::ElectronEnergy]) * (TeMid - TeEdge) * v * v;
  Real2nd Ti = TiEdge + tfac(growth_factors[Channel::IonEnergy]) *
                            (TiMid - TiEdge) * v * v;
  ...
  default:
    throw std::runtime_error(
        "Request for initial value for undefined variable!");
  }
}
...
Real MirrorPlasma::Flux(Index i, RealVector u, RealVector q, Real x, Time t) {
  ...
  case Channel::Density: {
    if (evolveLogDensity)
      return ConstantChannelMap[Channel::Density]
                 ? 0.0
                 : static_cast<Real>(Gamma(u, q, x, t) /
                                     uToDensity(u(Channel::Density)));
    else
      return ConstantChannelMap[Channel::Density]
                 ? 0.0
                 : static_cast<Real>(Gamma(u, q, x, t));
  ...
  case Channel::IonEnergy:
    return ConstantChannelMap[Channel::IonEnergy]
               ? 0.0
               : static_cast<Real>(
                     qi(u, q, x, t) +
                     lowPDiffusivity * (x / (xR - xL)) *
                         pow(q(Channel::IonEnergy) / u(Channel::IonEnergy), 1));
  ...
  case Channel::ElectronEnergy:
    ...
    break;
  case Channel::AngularMomentum:
    return ConstantChannelMap[Channel::AngularMomentum]
               ? 0.0
               : static_cast<Real>(Pi(u, q, x, t));
    break;
  default:
...
Real MirrorPlasma::Source(Index i, RealVector u, RealVector q, RealVector sigma,
                          RealVector phi, RealVector Scalars, Real x, Time t) {
  Channel c = static_cast<Channel>(i);
  Real S;
  ...
  case Channel::Density: {
    if (evolveLogDensity) {
      S = ConstantChannelMap[Channel::Density]
              ? 0.0
              : static_cast<Real>(1 / uToDensity(u(Channel::Density)) *
                                  (Sn(u, q, sigma, phi, x, t) +
                                   Gamma(u, q, x, t) * q(Channel::Density)));
      return S; // Avoid source cap when evolving log
    } else {
      S = ConstantChannelMap[Channel::Density] ? 0.0
                                               : Sn(u, q, sigma, phi, x, t);
  ...
  case Channel::IonEnergy:
    S = ConstantChannelMap[Channel::IonEnergy] ? 0.0
                                               : Spi(u, q, sigma, phi, x, t);
    break;
  case Channel::ElectronEnergy:
    S = ConstantChannelMap[Channel::ElectronEnergy]
            ? 0.0
            : Spe(u, q, sigma, phi, x, t);
    break;
  case Channel::AngularMomentum:
    S = ConstantChannelMap[Channel::AngularMomentum]
            ? 0.0
            : Somega(u, q, sigma, phi, Scalars, x, t);
    break;
  default:
    throw std::runtime_error("Request for flux for undefined variable!");
  }
...
Value MirrorPlasma::LowerBoundary(Index i, Time t) const { return uL[i]; }
Value MirrorPlasma::UpperBoundary(Index i, Time t) const { return uR[i]; }
...
// c.f Helander & Sigmar -- Gamma_e = (n_e T_e / (m_e Omega_e^2 tau_e))*( (p_e'
// + p_i')/p_e - (3/2)(T_e'/T_e) Define lengths so R_ref = 1
Real MirrorPlasma::Gamma(RealVector u, RealVector q, Real V, Time t) const {
  Real n = uToDensity(u(Channel::Density)),
       p_e = (2. / 3.) * u(Channel::ElectronEnergy),
       p_i = (2. / 3.) * u(Channel::IonEnergy);
  Real Te = p_e / n;
  Real Ti = p_i / n;
  Real nPrime = qToDensityGradient(q(Channel::Density), u(Channel::Density)),
       p_e_prime = (2. / 3.) * q(Channel::ElectronEnergy),
       p_i_prime = (2. / 3.) * q(Channel::IonEnergy);

  Real ThermalForce =
      (3. / 2.) * (p_e_prime - nPrime * Te); // p_e  * (1/T) * dT/dpsi
  ...
  Real U = (p_e_prime + p_i_prime) +
           n * omega * R * R * dOmegadV; // -n*(U_e - U_i) dot grad phi

  Real GeometricFactor =
      (B->VPrime(V) *
       R); // |grad psi| = R B , cancel the B with the B in Omega_e
  Real Gamma = GeometricFactor * GeometricFactor *
               (1 / (Plasma->ElectronCollisionTime(n, Te))) *
               (U - ThermalForce);

  // If gradient is too steep for MaNTA to handle, add diffusion

  Real rhon =
      abs(sqrt(Ti) * Plasma->RhoStarRef() * nPrime / B->dRdV(V, 0.0) / n);
  ...
  if (x >= 0)
    Gamma += x * lowNDiffusivity * nPrime * GeometricFactor *
             R; // SmoothTransition(x, transitionLength, lowNDiffusivity) *
                // Chi_n * GeometricFactor * R * nPrime * x;
  ...
  if (std::isfinite(Gamma.val))
    ...
    throw std::logic_error(
        ...
        std::to_string(V.val) + " and t = " + std::to_string(t));
};
...
/*
        ...
        ( n_i T_i / m_i Omega_i^2 tau_i ) * ( m_e Omega_e_ref^2 tau_e_ref / n0
   T0 ) = sqrt( m_i/2m_e ) * p_i / tau_i
*/
Real MirrorPlasma::qi(RealVector u, RealVector q, Real V, Time t) const {
  ...
  Real R = B->R_V(V, 0.0);
  Real GeometricFactor =
      (B->VPrime(V) * R); // |grad psi| = R B , cancel the B with the B in
                          // Omega_e, leaving (V'R)^2
  Real HeatFlux = 2.0 * GeometricFactor * GeometricFactor *
                  sqrt(Plasma->mu() / 2.0) *
                  (p_i / (Plasma->IonCollisionTime(n, Ti))) * Ti_prime;

  Real J = n * R * R; // Normalisation includes the m_i
  Real L = u(Channel::AngularMomentum);
  Real omega = L / J;
  Real PotentialEnergyFlux = 0.5 * omega * omega * R * R * Gamma(u, q, V, t);
  // Real lambda_p = abs(p_i_prime / p_i);

  // Real x = lambda_p - lowPThreshold;

  // if (lambda_p > lowPThreshold)
  // 	HeatFlux += SmoothTransition(x, transitionLength, lowPDiffusivity) *
  // p_i_prime / p_i; // Ti_prime / Ti;

  // Real lambda_T = (B->L_V(V) * (R_Upper - R_Lower)) * abs(Ti_prime);

  Real rhoTi =
      abs(sqrt(Ti) * Plasma->RhoStarRef() * Ti_prime / B->dRdV(V, 0.0) / Ti);
  ...
  // // Real x = (rhoTi - lowPThreshold) / lowPThreshold;
  // Real x = R_LTi - R_LTi_crit;
  // // Real lambda_n = 1 / B->dRdV(V 0.0) * abs(nPrime / n);

  // // Real x = sqrt(Ti) * lambda_n / (lowNThreshold / Plasma->RhoStarRef())
  // - 1.0;

  // // Real Chi_i = 1.0; // sqrt(Plasma->mu()) * pow(Ti, 3. / 2.) * lambda_T;

  // // if (x >= 0)
  // // 	HeatFlux += x * lowPDiffusivity * Chi_i * GeometricFactor *
  // GeometricFactor * n * pow(Ti, 3. / 2.) * Ti_prime; // / Ti; if (x >= 0)
  // 	HeatFlux += lowPDiffusivity * sqrt(Plasma->mu() / 2.0) * pow(Ti, 3.
  // / 2.) * pow(x, 2) * n * Ti_prime * GeometricFactor * GeometricFactor;
  ...
  if (std::isfinite(HeatFlux.val) && std::isfinite(PotentialEnergyFlux.val))
    ...
    throw std::logic_error(
        "Non-finite value computed for the ion heat flux at x = " +
        std::to_string(V.val) + " and t = " + std::to_string(t));
}

/*
   Following Helander & Sigmar, we have
   V' q_e = n_e T_e * V'^2 B^2 R^2 * ( T_e / m_e Omega_e^2 tau_e ) * ( 4.66
   T_e'/T_e - (3/2) * (p_e'+p_i')/p_e )
 */
Real MirrorPlasma::qe(RealVector u, RealVector q, Real V, Time t) const {
  ...
  Real U = Te * (p_e_prime + p_i_prime) +
           p_e * omega * R * R * dOmegadV; // -p_e*(U_e-U_i)

  Real GeometricFactor =
      (B->VPrime(V) * R); // |grad psi| = R B , cancel the B with the B in
                          // Omega_e, leaving (V'R)^2
  Real HeatFlux = GeometricFactor * GeometricFactor *
                  (1 / (Plasma->ElectronCollisionTime(n, Te))) *
                  (4.66 * p_e * Te_prime - (3. / 2.) * U);

  // Real Chi_e = pow(Te, 3. / 2.) * abs(1 / dRdV * Te_prime / Te);
  HeatFlux += TeDiffusivity * GeometricFactor * GeometricFactor * n * Te_prime;

  if (std::isfinite(HeatFlux.val))
    return HeatFlux;
  else
    throw std::logic_error(
        "Non-finite value computed for the electron heat flux at x = " +
        std::to_string(V.val) + " and t = " + std::to_string(t));
};
...
/*
   ...
   The Momentum Equation is normalised by n0^2 * T0 * m_i * c_s0 / ( m_e
   Omega_e_ref^2 tau_e_ref ) with c_s0 = sqrt( T0/mi )
...
Real MirrorPlasma::Pi(RealVector u, RealVector q, Real V, Time t) const {
  Real n = uToDensity(u(Channel::Density)),
       p_i = (2. / 3.) * u(Channel::IonEnergy);
  Real Ti = floor(p_i / n, MinTemp);
  // dOmega dV = L'/J - J' L / J^2 ; L = angular momentum / J = moment of
  // Inertia
  Real R = B->R_V(V, 0.0);
  ...
  Real Pi_v = IonClassicalAngularMomentumFlux(V, n, Ti, omega, dOmegadV, t) +
              omega * R * R * Gamma(u, q, V, t);
  ...
  // // 	Pi_v += SmoothTransition(x, transitionLength, lowLDiffusivity) *
  // LPrime / L; Real GeometricFactor = (B->VPrime(V) * R * R);
  // // Real x = sqrt(Ti) * lambda_omega / (lowLThreshold /
  // Plasma->RhoStarRef()) - 1.0;
  ...
  // // Real Chi_L = 1.0; // sqrt(Plasma->mu()) * pow(Ti, 3. / 2.) *
  // lambda_omega; Real Chi_L = 1.0; // sqrt(Plasma->mu()) * pow(Ti, 3. / 2.) *
  // lambda_omega;

  // Real rho_omega = abs(sqrt(Ti) * Plasma->RhoStarRef() * dOmegadV /
  ...
  // Pi_v += DiffuseHighGradient(omega, dOmegadV, lowLThreshold,
  // lowLDiffusivity, V);
  return Pi_v;
};
...
/*
   Returns V' pi_cl_i
 */
Real MirrorPlasma::IonClassicalAngularMomentumFlux(Real V, Real n, Real Ti,
                                                   Real omega, Real dOmegadV,
                                                   Time t) const {
  Real R = B->R_V(V, 0.0);
  Real GeometricFactor =
      (B->VPrime(V) * R * R); // |grad psi| = R B , cancel the B with the B in
                              // Omega_e, leaving (V'R)^2
  Real MomentumFlux = 0.3 * GeometricFactor * GeometricFactor *
                      sqrt(Plasma->mu() / 2.0) *
                      (n * Ti / (Plasma->IonCollisionTime(n, Ti))) * dOmegadV;

  if (std::isfinite(MomentumFlux.val))
    return MomentumFlux;
  else
    throw std::logic_error(
        "Non-finite value computed for the ion momentum flux at x = " +
        std::to_string(V.val) + " and t = " + std::to_string(t));
}

Real MirrorPlasma::Sn(RealVector u, RealVector q, RealVector sigma,
                      RealVector phi, Real V, Time t) const {
  Real n = uToDensity(u(Channel::Density)),
       p_e = (2. / 3.) * u(Channel::ElectronEnergy),
       p_i = (2. / 3.) * u(Channel::IonEnergy);
  ...
  if (useAmbipolarPhi) {
    ...
    Xi = CentrifugalPotential(V, omega, Ti, Te) +
  ...
  if (useNeutralModel)
    DensitySource = ParticleSource(R.val, t) +
                    Plasma->NormalizingTime() / n0 *
                        (Plasma->IonizationRate(n, NeutralDensity(R, t),
                                                R * omega, Te, Ti));
  else
    DensitySource = ParticleSource(R.val, t);
  ...
  auto EdgeFactor = [&](double Factor) {
    return (Factor +
            (1 - Factor) * pow(sin(xn / transitionLength * M_PI_2), 1.0));
  };
 ...
 */
Real MirrorPlasma::Spi(RealVector u, RealVector q, RealVector sigma,
                       RealVector phi, Real V, Time t) const {
  Real n = uToDensity(u(Channel::Density)),
       p_e = (2. / 3.) * u(Channel::ElectronEnergy),
       p_i = (2. / 3.) * u(Channel::IonEnergy);
  ...
  // Real PotentialHeating = IonPotentialHeating(u, q, phi, V); //-G * (omega *
  // omega / (2 * pi * a) - dphidV(u, q, phi, V));
  ...
  // if (useNeutralModel)
  // {
  ChargeExchangeHeatLosses =
      Ti * Plasma->NormalizingTime() / n0 *
      Plasma->ChargeExchangeLossRate(n, NeutralDensity(R, t), R * omega, Ti);
  // 	// ParticleSourceHeating = 0.5 * omega * omega * R * R *
  // (ParticleSource(R.val, t) + Plasma->NormalizingTime() / n0 *
  // (Plasma->IonizationRate(n, NeutralDensity(R, t), R * omega, Te, Ti) -
  // Plasma->ChargeExchangeLossRate(n, NeutralDensity(R, t), R * omega, Ti)));
  // // }
  // auto RampupFactor = [&](Time t)
  ...
  Real S = Heating -
           (ParallelLosses + ChargeExchangeHeatLosses + ParticleSourceHeating);
  auto RampupFactor = [&](Time t) {
    return SourceRampup > 0 ? (1 - 1e-2) * tanh(t / SourceRampup) + 1e-2 : 1.0;
  };
  S *= RampupFactor(t);
...
Real MirrorPlasma::Spe(RealVector u, RealVector q, RealVector sigma,
  ...
  Real AlphaHeating =
      sqrt(1 - 1 / MirrorRatio) * Plasma->TotalAlphaPower(n, p_i);
  ...
  // Real PotentialHeating = //ElectronPotentialHeating(u, q, phi, V);
  // //(dphi1dV(u, q, phi(0), V));
  ...
  Real PotentialHeating =
      0.0; // Te * Xi * ParticleSource(R.val, t); // Sn(u, q, sigma, phi, V, t);
  Real Heating = AlphaHeating + PotentialHeating;
  // Parallel Losses
  Real ParticleEnergy = Te * (1.0 + Xi);
  Real ParallelLosses =
      ParticleEnergy * ElectronPastukhovLossRate(V, Xi, n, Te);

  Real RadiationLosses =
      Plasma->BremsstrahlungLosses(n, p_e) + Plasma->CyclotronLosses(V, n, Te);
  ...
  // Real xn = (R - R_Lower) / (R_Upper - R_Lower);
  // auto EdgeFactor = [&](double Factor)
  // { return (Factor + (1 - Factor) * pow(sin(xn / transitionLength *
  // M_PI_2), 1.0)); }; if (xn <= transitionLength)
  // {
...
// account for the particle source being a sink later).
Real MirrorPlasma::Somega(RealVector u, RealVector q, RealVector sigma,
                          RealVector phi, RealVector Scalars, Real V,
                          Time t) const {
  // J x B torque
  Real R = B->R_V(V, 0.0);
  ...
  Real ParallelLosses =
      AngularMomentumPerParticle * IonPastukhovLossRate(V, Xi, n, Te);

  Real ChargeExchangeMomentumLosses = 0;
  // if (useNeutralModel)
  ChargeExchangeMomentumLosses =
      AngularMomentumPerParticle * Plasma->NormalizingTime() / n0 *
      Plasma->ChargeExchangeLossRate(n, NeutralDensity(R, t), R * omega, Ti);
...
template <typename T>
T MirrorPlasma::DiffuseHighGradient(T u, T q, double A, double D, T V) const {
  Real L = R_Upper - R_Lower;
  T Lu = abs(q * L / (B->dRdV(V, 0.0)));
  T c = A / L;
  T x = (Lu - c) / L;
  T G = 0.0;
  if (x >= 0) {
    // std::cerr << "Diffusing high gradient " << x << " at x = " << V <<
    // std::endl;
    G += D * x * q;
  }
  return G;
...
inline Real MirrorPlasma::AmbipolarPhi(Real V, Real n, Real Ti, Real Te) const {
  ...
  double Sigma = 1.0;
  return log(
      (Plasma->ElectronCollisionTime(n, Te) / Plasma->IonCollisionTime(n, Ti)) *
      (log(R * Sigma) / (Sigma * log(R))));
}

Real MirrorPlasma::ParticleSource(double R, double t) const {
  double shape = 1 / ParticleSourceWidth;
  // // return (exp(-shape * (R - R_Lower) * (R - R_Lower)) + exp(-shape * (R -
  // R_Upper) * (R - R_Upper))); Real S = LowerParticleSourceStrength * exp(-(R
  // - R_Lower) * shape) + UpperParticleSourceStrength * exp((R - R_Upper) *
  // shape);
  // // return LowerParticleSourceStrength; //* exp(-t / 5e-2);
  Real S =
      LowerParticleSourceStrength +
      UpperParticleSourceStrength *
          exp(-shape * (R - ParticleSourceCenter) * (R - ParticleSourceCenter));
...
Real MirrorPlasma::ElectronPastukhovLossRate(Real V, Real Xi_e, Real n,
                                             Real Te) const {
  Real MirrorRatio = B->MirrorRatio(V, 0.0);
  Real tau_ee = Plasma->ElectronCollisionTime(n, Te);
  double Sigma = 1 + Z_eff; // Include collisions with ions and impurities as
                            // well as self-collisions
  ...
  double Normalization =
      Plasma->mu() * (1.0 / (Plasma->RhoStarRef() * Plasma->RhoStarRef()));
  Real LossRate = (M_2_SQRTPI / tau_ee) * Normalization * Sigma * n *
                  (1.0 / log(MirrorRatio * Sigma)) * PastukhovFactor;
  // if (Xi_e < 1.0)
  // 	LossRate = Plasma->NormalizingTime() * n * sqrt(Te * T0 /
  // Plasma->IonMass()) / B->L_V(V);
  return LossRate;
}
...
Real MirrorPlasma::IonPastukhovLossRate(Real V, Real Xi_i, Real n,
                                        Real Ti) const {
  ...
  double Normalization = sqrt(Plasma->mu()) *
                         (1.0 / (Plasma->RhoStarRef() * Plasma->RhoStarRef()));
  Real LossRate = (M_2_SQRTPI / tau_ii) * Normalization * Sigma * n *
                  (1.0 / log(MirrorRatio * Sigma)) * PastukhovFactor;
  // if (Xi_i < 1.0)
  // 	LossRate = Plasma->NormalizingTime() * n * sqrt(Ti * T0 /
  // Plasma->IonMass()) / B->L_V(V);
  return LossRate;
}

Real MirrorPlasma::ViscousHeating(RealVector u, RealVector q, Real V,
                                  Time t) const {
  Real n = uToDensity(
      u(Channel::Density)); //, p_i = (2. / 3.) * u(Channel::IonEnergy);
  // Real Ti = floor(p_i / n, MinTemp);
  //  pi * d omega / d psi = (V'pi)*(d omega / d V)
  Real R = B->R_V(V, 0.0);
  Real J = n * R * R; // Normalisation includes the m_i
  ...
  Real ViscousHeating =
      Pi(u, q, V, t) *
      dOmegadV; //(IonClassicalAngularMomentumFlux(V, n, Ti, omega, dOmegadV, t)
                //+ omega * R * R * Gamma(u, q, V, t)) * dOmegadV;

  return ViscousHeating;
}

Real MirrorPlasma::IonPotentialHeating(RealVector u, RealVector q,
                                       RealVector phi, Real V) const {
  ...
  Real L = u(Channel::AngularMomentum);
  Real omega = L / J;
  return -0.5 * omega * omega * R * R *
         Sn(u, q, u, phi, V, 0.0); //-Gamma(u, q, V, 0.0) * (omega * omega / (2
                                   //* pi * a) - dphidV(u, q, phi, V));
}

Real MirrorPlasma::ElectronPotentialHeating(RealVector u, RealVector q,
                                            RealVector phi, Real V) const {
  return 0.0;
}
