...
Real PlasmaConstants::BremsstrahlungLosses(Real n, Real pe) const {
  ...
  Real Pbrem =
      NormalizingTime() / (n0 * T0) * 5.34e3 * sqrt(pe / n) * Z_eff * n * n;
  return Pbrem;
};
...
Real PlasmaConstants::IonizationRate(Real n, Real NeutralDensity, Real v,
                                     Real Te, Real Ti) const {

  Real n_m3 = n * n0;
  Real n_neutrals = NeutralDensity * n0;
  try {
    Real IonIntegral = NeutralProcess(
        [this](double Energy) {
          return Plasma->protonImpactIonizationCrossSection(Energy);
        },
        v, Ti, IonMass(), 200.0);
...
Real PlasmaConstants::ChargeExchangeLossRate(Real n, Real NeutralDensity,
                                             Real v, Real Ti) const {
  Real n_m3 = n * n0;
  Real n_neutrals = NeutralDensity * n0;
  try {
    Real IonIntegral = NeutralProcess(
        [this](double Energy) {
          return Plasma->HydrogenChargeExchangeCrossSection(Energy);
        },
        v, Ti, IonMass(), 0.1);
    ...
    throw std::runtime_error(
        "Caught exception while trying to caluclat eCharge exchange");
  }
};
 ...
 */
Real PlasmaConstants::IonElectronEnergyExchange(Real n, Real pe, Real pi,
                                                Real V, double t) const {
  Real Te = pe / n;
  double RhoStar = RhoStarRef();
  Real pDiff = pe - pi;
  Real IonHeating = (pDiff / (ElectronCollisionTime(n, Te))) *
                    (3.0 / (RhoStar * RhoStar)); //* (ElectronMass / IonMass));

  if (std::isfinite(IonHeating.val))
    return IonHeating;
  else
    throw std::logic_error(
        "Non-finite value computed for the ion heating at x = " +
...
double PlasmaConstants::ReferenceElectronCollisionTime() const {
  double LogLambdaRef = 24.0 - log(n0cgs) / 2.0 +
                        log(T0eV); // 24 - ln( n^1/2 T^-1 ) from NRL pg 34
  return 12.0 * pow(M_PI, 1.5) * sqrt(ElectronMass) * pow(T0, 1.5) *
         VacuumPermittivity * VacuumPermittivity /
         (sqrt(2) * n0 * pow(ElementaryCharge, 4) * LogLambdaRef);
};

// Return the actual value in SI units
double PlasmaConstants::ReferenceIonCollisionTime() const {
  double LogLambdaRef =
      23.0 - log(2.0) - log(n0cgs) / 2.0 +
      log(T0eV) * 1.5; // 23 - ln( (2n)^1/2 T^-3/2 ) from NRL pg 34
  return 12.0 * pow(M_PI, 1.5) * sqrt(IonMass()) * pow(T0, 1.5) *
         VacuumPermittivity * VacuumPermittivity /
         (n0 * pow(ElementaryCharge, 4) * LogLambdaRef);
}
...
// mi/me
double PlasmaConstants::mu() const { return IonMass() / ElectronMass; }
