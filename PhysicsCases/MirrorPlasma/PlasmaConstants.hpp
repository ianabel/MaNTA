...
// Base ion species class for species-specific values, for now just has the
// fusion rate and mass
class IonSpecies {
public:
  // IonSpecies() = default;
  ...
  virtual Real electronImpactIonizationCrossSection(Real Energy) const {
    ...
    constexpr std::array<double, 5> fittingParamB{-0.032226, -0.034539, 1.4003,
                                                  -2.8115, 2.2986};
    ...
    if (Energy < minimumEnergySigma) {
      ...
      if (x <= 0)
        return 0.0;
      sum = x * (fittingParamB[0] +
                 x * (fittingParamB[1] +
                      x * (fittingParamB[2] +
                           x * (fittingParamB[3] + x * fittingParamB[4]))));
      sigma = (1.0e-13 / (ionizationEnergy * Energy)) *
              (fittingParamA * log(Energy / ionizationEnergy) + sum);
      return sigma;
    }
  }
  ...
  virtual Real protonImpactIonizationCrossSection(Real Energy) const {
    ...
    constexpr double minimumEnergySigma = 0.5;
    // Convert to keV
    ...
    if (EnergyKEV < minimumEnergySigma) {
      ...
      sigma = 1e-16 * A1 *
              (exp(-A2 / EnergyKEV) * log(1 + A3 * EnergyKEV) / EnergyKEV +
               A4 * exp(-A5 * EnergyKEV) /
                   (pow(EnergyKEV, A6) + A7 * pow(EnergyKEV, A8)));
    }
    return sigma;
  }
  ...
  virtual double HydrogenChargeExchangeCrossSection(double Energy) {
    ...
    if (Energy < minimumEnergySigma_n1) {
      ...
      sigma_n1 = 1e-16 * 3.2345 * log(235.88 / EnergyKEV + 2.3713) /
                 (1 + 0.038371 * EnergyKEV + 3.8068e-6 * pow(EnergyKEV, 3.5) +
                  1.1832e-10 * pow(EnergyKEV, 5.4));
    }
    return sigma_n1;
  }
};
...
public:
  ...
  Real FusionRate(Real n, Real pi) const override { return 0.0; }
...
public:
  ...
  Real FusionRate(Real n, Real Ti) const override {
    ...
    if (Ti < 25)
      ...
      sigmav = 1e-6 * 2.7e-16; // Formula is only valid for Ti < 25 keV, just
                               // set to a constant after
...
template <typename T> constexpr IonSpecies *createIonSpecies() {
  return new T();
};

// Map to the constructor for each of the different ion species
static std::map<std::string, IonSpecies *(*)()> PlasmaMap = {
    {"Hydrogen", &createIonSpecies<Hydrogen>},
    {"Deuterium", &createIonSpecies<Deuterium>},
    {"DeuteriumTritium", &createIonSpecies<DeuteriumTritium>}};

// Class for computing common plasma constants
class PlasmaConstants {
public:
  // Constructor takes ion type, magnetic field, other normalizing parameters if
  // desired
  PlasmaConstants(std::string IonType, std::shared_ptr<MagneticField> B,
                  double PlasmaWidth)
      : B(B), PlasmaWidth(PlasmaWidth) {
  ...
  PlasmaConstants(std::string IonType, std::shared_ptr<MagneticField> B,
                  double n0, double T0, double Z_eff, double PlasmaWidth)
      : B(B), n0(n0), T0(T0), Z_eff(Z_eff), PlasmaWidth(PlasmaWidth) {
    auto it = PlasmaMap.find(IonType);
    if (it == PlasmaMap.end())
      throw std::logic_error("Requested ion species does not exist.");
    Plasma = it->second();
  ...
  template <typename T> T LogLambda_ii(T ni, T Ti) const {
  ...
  Real IonizationRate(Real n, Real NeutralDensity, Real v, Real Te,
                      Real Ti) const;
  Real ChargeExchangeLossRate(Real n, Real NeutralDensity, Real v,
                              Real Ti) const;

  Real IonElectronEnergyExchange(Real n, Real pe, Real pi, Real V,
                                 double t) const;
...
private:
  ...
  // Calculates the velocity space integral for collisional processes with
  // neutrals CrossSection is a lambda with units of cm^2 that returns an
  // autodiff compatible type (Real or double)
  template <class XS>
  Real NeutralProcess(XS const &CrossSection, Real vtheta, Real T, double Mass,
                      double minEnergy,
                      MomentTypes Moment = MomentTypes::Density) const {
    ...
    auto Integrand = [&](double Energy, Real Mass, Real M, Real T,
                         Real CrossSection) {
      Real MmE = M - sqrt(Energy / T);
      Real MpE = M + sqrt(Energy / T);
      Real v = sqrt(2 * ElementaryCharge * Energy / Mass);

      // For energy and momentum losses, we need to calculate the appropriate
      // moment
      Real x;
      switch (Moment) {
      ...
      Real I = v * x * ElementaryCharge / Mass * (CrossSection * 1e-4) *
               (exp(-MmE * MmE) - exp(-MpE * MpE));
      return I;
    };
    ...
    Real minE = minEnergy; // max(minEnergy, min_velocity * min_velocity * T *
                           // T0 / ElementaryCharge);
    ...
    if (maxE <= minE || !std::isfinite(maxE.val))
      return Integral; // Return 0 if the domain of integration doesn't make
                       // sense

    try {
      Integral.val = integrator::integrate(
          [&](double Energy) {
            return Integrand(Energy, Mass, Mth, T * T0eV, CrossSection(Energy))
                .val;
          },
    ...
    if (T.grad != 0 || vtheta.grad != 0) {
      Real dmaxEdT = maxEV > 1e6
                         ? 0.0
                         : static_cast<Real>(T0 / ElementaryCharge *
                                             (max_velocity * max_velocity -
                                              2 * Mth * max_velocity));
      Real dmaxEdVtheta =
          maxEV > 1e6 ? 0.0
                      : static_cast<Real>(-2 * Mth / vtheta * max_velocity * T *
                                          T0 / ElementaryCharge);
      Integral.grad = integrator::integrate(
          [&](double Energy) {
            return Integrand(Energy, Mass, Mth, T * T0eV, CrossSection(Energy))
                .grad;
          },
          minE.val, maxE.val, max_depth, tol);
      if (T.grad != 0)
        Integral.grad += dmaxEdT.val * Integrand(maxE.val, Mass, Mth, T * T0eV,
                                                 CrossSection(maxE.val))
                                           .val;
      else
        Integral.grad +=
            dmaxEdVtheta.val *
            Integrand(maxE.val, Mass, Mth, T * T0eV, CrossSection(maxE.val))
                .val;
    }
