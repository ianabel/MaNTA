...
BOOST_AUTO_TEST_CASE( nodal_basis_test )
{
    ...
    std::vector<double> nodes{ -std::cos( M_PI/10 ), -std::cos( 3 * M_PI / 10 ), 0, std::cos( 3 * M_PI / 10 ), std::cos( M_PI / 10 )};
    std::vector<double> test_points{ -1.0, -0.5, 0.0, 0.5, 1.0 };


    Eigen::MatrixXd Vals( 5, 5 );
    Vals <<  1.26275,  -0.0685096, 0.0,  0.021296,     0.0316769,
            -0.392522,  0.921586,  0.0,  -0.0743729,  -0.101905,
             0.2,       0.2,       1.0,  0.2,          0.2,
            -0.101905, -0.0743729, 0.0,  0.921586,    -0.392522,
             0.0316769, 0.021296,  0.0,  -0.0685096,   1.26275;

    Eigen::MatrixXd PrimeVals( 5, 5 );
    PrimeVals <<   -4.9798,   -1.05146,    0.32492,  -0.248217,   0.525731,
                    7.20683,  -0.449028,  -1.37638,   0.850651,  -1.7013,
                   -3.4026,    2.10292,    0.0,      -2.10292,    3.4026,
                    1.7013,   -0.850651,   1.37638,   0.449028,  -7.20683,
                   -0.525731,  0.248217,  -0.32492,   1.05146,    4.9798;
    ...
    for( int i=0; i < 5; i++ ) {
        ...
        for( int j=0; j < 5; j++ ) {
            ...
            BOOST_TEST( basis.Evaluate( i, test_points[ j ] ) == Vals( i, j ), boost::test_tools::tolerance( 1e-6 ) );
...
BOOST_AUTO_TEST_CASE( dg_approx_static_nodal )
{
    ...
    ref <<  0.149562,    0.0339152,  -0.0248848,   0.0168785,  -0.00768981,
            0.0339152,   0.467581,    0.0375832,  -0.0304054,   0.0168785,
           -0.0248848,   0.0375832,   0.587937,    0.0375832,  -0.0248848,
            0.0168785,  -0.0304054,   0.0375832,   0.467581,    0.0339152,
           -0.00768981,  0.0168785,  -0.0248848,   0.0339152,   0.149562;
    ...
    ref <<   0.000228649,  -0.0000807268,  -0.0000190304,  0.000119571,  -0.000120153,
            -0.0000807268,  0.00329087,     0.000242068,  -0.000475085,   0.000407881,
            -0.0000190304,  0.000242068,    0.00918651,    0.000932406,  -0.000758618,
    ...
    basis.DerivativeMatrix( testGrid[ 0 ], tmp );
    // Courtesy of Mathematica 
    ref <<  -0.796767,    1.12433,   -0.499239,   0.254347,  -0.0826722,
            -0.631902,   -0.0718445,  0.989258,  -0.423612,   0.138101,
             0.253024,   -0.931135,   0.0,        0.931135,  -0.253024,
            -0.138101,    0.423612,  -0.989258,   0.0718445,  0.631902,
             0.0826722,  -0.254347,   0.499239,  -1.12433,    0.796767;
