...
BOOST_AUTO_TEST_CASE(global_weights_integrate_exactly)
{
    ...
    const Vector &w = Integrator::getIntegrationWeights(basis, grid);
...
BOOST_AUTO_TEST_CASE(weights_are_rebuilt_when_the_grid_changes)
{
    ...
    const Vector w1 = Integrator::getIntegrationWeights(basis, first);
    ...
    const Vector &w2 = Integrator::getIntegrationWeights(basis, second);
    ...
    const Vector &w3 = Integrator::getIntegrationWeights(basis, third);
    BOOST_TEST(integrate(w3, third, k, one) == 2.0);

    // Going back to the first grid must still give the original answer.
    const Vector &w1again = Integrator::getIntegrationWeights(basis, first);
...
BOOST_AUTO_TEST_CASE(weights_are_rebuilt_when_the_order_changes)
{
    ...
    const Vector w3 = Integrator::getIntegrationWeights(b3, grid);
    ...
    const Vector &w6 = Integrator::getIntegrationWeights(b6, grid);
...
BOOST_AUTO_TEST_CASE(phi_boundary_is_rebuilt_when_the_grid_changes)
{
    ...
    const Matrix p1 = Integrator::getPhiBoundary(basis, first);
    ...
    const Matrix &p2 = Integrator::getPhiBoundary(basis, second);
