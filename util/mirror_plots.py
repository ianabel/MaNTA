...
data = Dataset("../runs/CMFX3.nc")  # xr.open_dataset("./Config/LinearDiffusion.nc")
...
p_i = 2.0 / 3.0 * np.array(data.groups["Var1"].variables["u"])
p_e = 2.0 / 3.0 * np.array(data.groups["Var2"].variables["u"])
dp_i = 2.0 / 3.0 * np.array(data.groups["Var1"].variables["q"])
dp_e = 2.0 / 3.0 * np.array(data.groups["Var2"].variables["q"])
...
nAD = np.array(
    data.groups["ArtificialDiffusion"].variables["DensityArtificialDiffusion"]
)
piAD = np.array(
    data.groups["ArtificialDiffusion"].variables["IonPressureArtificialDiffusion"]
)
peAD = np.array(
    data.groups["ArtificialDiffusion"].variables["ElectronPressureArtificialDiffusion"]
)
LAD = np.array(
    data.groups["ArtificialDiffusion"].variables["AngularMomentumArtificialDiffusion"]
)
...
pfluxderiv = np.abs((dp_i) / p_e + 3.0 / 2.0 * dn / n) - np.abs(0.5 * dp_e / p_e)
...
ax.legend()
ax.set_title(
    rf"Angular momentum density (${round(ionMass * n0 * omega0, 2)}$  $kg/m\cdot s$)"
)
# ax.set_xlabel(r"$\Hat{V}$")
ax.set_xlabel(r"$r(m)$")
ax.set(adjustable="box")
...
ax.legend()
ax.set_title(rf"Ion and electron pressure (${round(p0 / 1000, 2)}$  $kPa$)")
# ax.set_xlabel(r"$\Hat{V}$")
ax.set_xlabel(r"$r(m)$")
ax.set(adjustable="box")
...
PLpar = (
    2
    * np.pi
    * np.trapezoid(Ldev * Lnorm * r * p0 / (taunorm) * omega[-1, :] * Lpar[-1, :], r)
)
Ptorque = (
    2
    * np.pi
...
StoredIonEnergy = (
    3.0 / 2.0 * 2 * np.pi * np.trapezoid(Ldev * Lnorm * r * p_i[-1, :] * p0, r)
)
StoredElectronEnergy = (
    3.0 / 2.0 * 2 * np.pi * np.trapezoid(Ldev * Lnorm * r * p_e[-1, :] * p0, r)
)
...
print(
    ...
    Pvis
    + Palpha
    - np.abs(Pipar + Pepar + PclasI + PclasE + Prad + PCX + Pcyc - PpotI - PpotE),
)
print(
    "Energy confinement time (ms): ",
    1000 * (StoredElectronEnergy + StoredIonEnergy) / (IonHeatLoss + ElectronHeatLoss),
)
