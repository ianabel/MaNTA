...
class StellaratorTransport(MaNTA.TransportSystem):
    def __init__(self, config, yancc_wrapper: yancc_data):
        ...
        if use_chunking:
            self.chunk_size = (
                2 * len(devices)
                if jnp.mod(len(self.points), 2 * len(devices)) == 0
                else len(devices)
            )
            print(
                "Using chunking with chunk size "
                + str(self.chunk_size)
                + " on "
                + str(len(devices))
                + " devices"
...
class StellaratorAdjointProblem(MaNTA.AdjointProblem):
    ...
    def dSigma(self, i, states, positions):
        ...
        # compute gradient
        fgrad = eqx.filter_grad(
            lambda tree, s, p: self.sigma(
                i, s, p, 0, tree[0], tree[1], tree[2], self.params
            ),
            has_aux=True,
        )
        grad_out = jax.lax.map(
            lambda args: fgrad(*args),
