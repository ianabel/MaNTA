...
int JacSetup(sunrealtype tt, sunrealtype cj, N_Vector yy, N_Vector yp,
             N_Vector rr, SUNMatrix Jac, void *user_data, N_Vector tmp1,
...
ISTATUS SystemSolver::initialize() {
  ...
  if (ErrorChecker::check_retval((void *)IDA_mem, "IDACreate", 0)) {
    logmsg<LOG_LEVEL::ERROR>("Sundials Initialization Error on line {}",
                             __LINE__ - 2);
  ...
  Y = N_VNew_Serial(nVars * 3 * nCells * (k + 1) + nVars * (nCells + 1) +
                        nScalars + nAux * nCells * (k + 1),
                    ctx);
  ...
  if (optimizeMode) {
    ...
    logmsg<LOG_LEVEL::INFO>("Initial dGdt = {}", dGdt(0));
    const double stoptol = 0.05; // acceptable decrease in G
    for (const auto &dG : dGdt) {
      const double deltaG = dt * dG;
      if ((deltaG < -stoptol) || (deltaG > 100.0)) {
        logmsg<LOG_LEVEL::WARNING>(
            "Negative value of dGdt = {} encountered; optimize mode assumes "
            "that we are maximizing G, so a negative dGdt indicates a bad "
            "step.",
            dG);
        return ISTATUS::NEGATIVE_DGDT;
      }
      //  else if (abs(dt * dG) < stoptol)
      // {
      //   logmsg<LOG_LEVEL::WARNING>("Small value of dGdt = {} encountered;
      //   returning previous value of G.", dG); return ISTATUS::ZERO_DGDT;
      // }
    }
  }
  ...
  // No constraints are imposed as negative coefficients may allow for a better
  // fit across a cell
  ...
  VectorWrapper absTolVals(N_VGetArrayPointer(absTolVec),
                           N_VGetLength(absTolVec));
  absTolVals.setZero();
  ...
  // give it a pointer to 'this' and it won't hold it beyond the lifetime of
  // this function call.
  ...
  if (physics_debug) {
    ...
    res_out << "# Residual norm at t = " << t0 << " (pre-calcIC) is "
            << residual_val << std::endl;
  ...
  logmsg<LOG_LEVEL::INFO>("Number of Residual Evaluations due to IDACalcIC: {}",
                          nresevals);

  if (nresevals > 10)
    logmsg<LOG_LEVEL::WARNING>(
        "IDACalcIC required {} residual evaluations. Check settings in {}",
        nresevals, std::string(inputFilePath));
  ...
  if (physics_debug) {
    ...
    res_out << "# Residual norm at t = " << t0 << " (post-CalcIC) is "
            << N_VWrmsNorm(res, wgt) << std::endl;
    printOnNodes(res_out, t0, res);
  }
  ...
  if (aggressiveTimesteps)
    IDASetEtaMax(IDA_mem, 10.0); // Default is 2.0
...
void SystemSolver::runSolver(double tFinal) {
  ...
  if (t0 > tFinal) {
    logmsg<LOG_LEVEL::ERROR>(
        "Initial time t = {} is after the end of the simulation at t = {}", t0,
        tFinal);
  ...
  while (tFinal - tret > min_step_size || TerminateOnSteadyState) {
    ...
    std::cout << "Writing output at " << tret << " ( " << nstep_tmp
              << " timesteps )" << std::endl;
    ...
    if (physics_debug) {
      ...
      IDAEwtSet(Y, wgt, IDA_mem);
      res_out << "# Residual norm at t = " << tret << " is "
              << N_VWrmsNorm(res, wgt) << std::endl;
    ...
    if (TerminateOnSteadyState) {
      ...
      if (physics_debug)
        std::cout << " dy/dt norm inferred from lambdas is " << dydt_norm
                  << std::endl;
      if (dydt_norm < 1.0) {
  ...
  std::cout << "Total Number of Residual Evaluations  :" << nresevals
            << std::endl;
  std::cout << "Total Number of Jacobian Computations :" << njacevals
            << std::endl;
...
void SystemSolver::runAdjointSolve() {
  if (solveAdjoint) {
    ...
    logmsg<LOG_LEVEL::ERROR>(
        "Error: runAdjointSolve called but \"solveAdjoint\" was set to false");
  }
}

void SystemSolver::destroySundials() {
  // No SunLinSol wrapper classes exist beyond this point, so we are safe in
  // using raw pointers to construct them.
  SUNLinSolFree(LS);
  if (sunMat)
    MatDestroy(sunMat);
 ...
 * This is the function that should set the point at which the sub-matrices for
 * the Jacobian solve are evaluated
 */
int JacSetup(sunrealtype tt, sunrealtype cj, N_Vector yy, N_Vector yp,
             N_Vector rr, SUNMatrix Jac, void *user_data, N_Vector tmp1,
             N_Vector tmp2, N_Vector tmp3) {
  // Sundials looks for a Jacobian, but our Jacobian equation is solved without
  // computing the jacobian. We use this function to capture t and cj for the
  // solve.
