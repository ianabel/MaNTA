...
ISTATUS SystemSolver::initialize() {
  ...
  if (ErrorChecker::check_retval((void *)IDA_mem, "IDACreate", 0)) {
    log<LOG_LEVEL::ERROR>("Sundials Initialization Error on line {}",
                          __LINE__ - 2);
  ...
  if (optimizeMode) {
    ...
    for (const auto &dG : dGdt) {
      if (dG < 0) {
        log<LOG_LEVEL::WARNING>("Negative value of dGdt = {} encountered; "
                                "Optimize mode assumes that we are maximizing "
                                "G, so a negative dGdt indicates a bad step.",
                                dG);
  ...
  log<LOG_LEVEL::INFO>("Number of Residual Evaluations due to IDACalcIC: {}",
                       nresevals);
